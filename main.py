...
class ImgTools(utils.BaseWindow):
    def __init__(self):
        ...
        # Will be used later in save (convert) function
    ...
    def create_dialogs(self):
        '''
        Method creates all dialogs once again.
        Should be called after changes made in any dialog window
        '''
    ...
    def enable_buttons(self, _):
        # When any dialog window hided all buttons will get enabled
    ...
    def disable_buttons(self, _):
        # When any dialog window shown all buttons will get disabled
    ...
    def optimize(self):
        ...
        # This var will be checked when saving file
    ...
    # > Editing image <
    def convert(self):
        '''Converts image to certain file format and saves it'''
        if not self.image_check():
            return
        ...
        # No prompt
        if nfn == '':
            self.statusBar().showMessage('File name can\'t be empty')
            return
        ...
        if ok_pressed:
            ...
            fn = '%s.%s' % (nfn, ff)
            self.img.save(
                # Join original filepath with new filename
                os.path.join(
                    self.fp,
                    fn
                ),
            ...
            self.statusBar().showMessage('Saved converted file: %s' % (
                # Showing user ✨ pretty ✨ path of new file
    ...
    def resize_callback(self, _):
        ...
        if not ratio:
            ...
            # Resize image with maintained aspect ration
            # If both (height and width) values changes
            # resizing w/out maintaining aspect ratio
            if w1 == w2:
    ...
    def censore_callback(self, _):
        ...
        # Generating censore effect on dialog's display image
        # and replacing self.img with it
